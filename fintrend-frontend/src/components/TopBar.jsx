            ...
            `}
              ...
              ">
                                <div className="px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50">
                                    <div className="flex justify-between items-center">
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500 mt-0.5">Real-time sentiment analysis</p>
                                        </div>
                                        <button
                                            onClick={() => setNotificationsOpen(false)}
                                ...
                                <div className="max-h-[450px] overflow-y-auto">
                                    {/* Sentiment Summary */}
                                    <div className="p-5 bg-gradient-to-br from-gray-50 to-blue-50/30 border-b border-gray-100">
                                        <div className="flex items-center justify-between mb-3">
                                            <span className="text-xs font-bold text-gray-600 uppercase tracking-wider">Market Mood</span>
                                            <span className="text-xs text-gray-500">{notifications.length} signals</span>
                                        </div>
                                        <div className="grid grid-cols-3 gap-2">
                                            <div className="bg-white rounded-lg p-3 text-center shadow-sm border border-green-100">
                                                <div className="text-2xl font-bold text-green-600">
                                                    {notifications.filter(n => n.sentiment === 'positive').length}
                                                </div>
                                                <div className="text-[10px] font-semibold text-gray-600 uppercase mt-1">Bullish</div>
                                            </div>
                                            <div className="bg-white rounded-lg p-3 text-center shadow-sm border border-gray-100">
                                                <div className="text-2xl font-bold text-gray-600">
                                                    {notifications.filter(n => n.sentiment === 'neutral').length}
                                                </div>
                                                <div className="text-[10px] font-semibold text-gray-600 uppercase mt-1">Neutral</div>
                                            </div>
                                            <div className="bg-white rounded-lg p-3 text-center shadow-sm border border-red-100">
                                                <div className="text-2xl font-bold text-red-600">
                                                    {notifications.filter(n => n.sentiment === 'negative').length}
                                                </div>
                                                <div className="text-[10px] font-semibold text-gray-600 uppercase mt-1">Bearish</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4">
                                        <div className="flex items-center justify-between mb-3">
                                            <span className="text-xs font-bold text-gray-600 uppercase tracking-wider">High Impact Events</span>
                                            {notifications.length > 0 && (
                                                <span className="text-xs text-blue-600 font-medium">Last hour</span>
                                            )}
                                        </div>
                                        ...
                                        {notifications.length === 0 ? (
                                            <div className="py-8 text-center">
                                                ...
                                                <p className="text-sm text-gray-500 font-medium">All quiet on the markets</p>
                                                <p className="text-xs text-gray-400 mt-1">No high-impact events detected</p>
                                            </div>
                                        ) : (
                                            <div className="space-y-2">
                                                {notifications.slice(0, 5).map((n) => (
                                                    <div key={n.id} className="group bg-white hover:bg-blue-50/50 rounded-xl p-3 border border-gray-100 hover:border-blue-200 transition-all cursor-pointer">
                                  ...
                                  `}>
                                                                ...
                                                                <p className="text-sm text-gray-800 font-medium leading-snug line-clamp-2 group-hover:text-blue-900 transition-colors">
                                                                    {n.title}
                                                                </p>
                                                            </div>
                                ...
                                <div className="p-3 bg-gradient-to-r from-gray-50 to-blue-50/30 text-center border-t border-gray-100">
                                    <button
                                        onClick={() => {
